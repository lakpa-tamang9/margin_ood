...
swa_model = AveragedModel(args.model)
...
def train(loader, model, criterion, optimizer):
    ...
    for _, (input, target) in enumerate(loader):
        ...
        criterion(model(input)[1], target).backward()
...
with open(
    os.path.join(
        logs_n_ckpt_dirs,
        args.dataset + "_" + args.model + "_" + "training_results.csv",
    ),
    "w",
) as f:
...
for epoch in range(0, args.epochs):
    ...
    train(train_loader_in, net, cls_criterion, optimizer)
    if scheduler != None:
        if epoch > swa_start:
